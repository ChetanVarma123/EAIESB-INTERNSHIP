...
async def create_section(section: Section):
    ...
    created_section = await (db[section_collection_name]
                             .find_one({"_id": result.inserted_id}))
    if created_section:
...
async def update_section(section_id: str, section: Section):
    ...
    if result.modified_count == 1:
        updated_section = await (db[section_collection_name]
                                 .find_one({"_id": ObjectId(section_id)}))
...
async def delete_section_by_id(section_id: str):
    db = await get_database()
    existing_section = await (db[section_collection_name]
                              .find_one({"_id": ObjectId(section_id)}))
    if existing_section is None:
        raise HTTPException(status_code=404, detail="Section not found")
    result = await (db[section_collection_name]
                    .delete_one({"_id": ObjectId(section_id)}))
    if result.deleted_count == 1:
