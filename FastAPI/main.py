...
async def get_campuses():
    ...
    return [CampusInDB(**campus,
                       id=str(campus["_id"])) for campus in campuses]
...
async def create_campus(campus: Campus):
    ...
    created_campus = await (db[campus_collection_name]
                            .find_one({"_id": result.inserted_id}))
    if created_campus:
        return CampusInDB(**created_campus,
                          id=str(created_campus["_id"]))
    else:
...
async def update_campus(campus_id: str, campus: Campus):
    ...
    if result.modified_count == 1:
        updated_campus = await (db[campus_collection_name]
                                .find_one({"_id": ObjectId(campus_id)}))
...
async def delete_campus_by_id(campus_id: str):
    db = await get_database()
    existing_campus = await (db[campus_collection_name]
                             .find_one({"_id": ObjectId(campus_id)}))
    if existing_campus is None:
        raise HTTPException(status_code=404,
                            detail="Campus not found")
    result = await (db[campus_collection_name]
                    .delete_one({"_id": ObjectId(campus_id)}))
...
async def get_departments():  # Modified
    await connect_to_mongo()
    db = await get_database()
    departments = await (db[department_collection_name]
                         .find({}).to_list(None))
    return [DepartmentInDB(**department, id=str(department["_id"]))
            for department in departments]
@app.post("/department", response_model=DepartmentInDB)
async def create_department(department: Department):
    db = await get_database()
    department_dict = department.dict()
    result = await (db[department_collection_name]
                    .insert_one(department_dict))
    created_department = await (db[department_collection_name]
                                .find_one({"_id": result.inserted_id}))
    if created_department:
        return DepartmentInDB(**created_department,
...
async def update_department(department_id: str, department: Department):
    ...
    if result.modified_count == 1:
        updated_department = await (db[department_collection_name]
                                    .find_one({"_id": ObjectId(department_id)}))
        return DepartmentInDB(**updated_department,
...
async def delete_department_by_id(department_id: str):
    db = await get_database()
    existing_department = await (db[department_collection_name]
                                 .find_one({"_id": ObjectId(department_id)}))
    if existing_department is None:
        raise HTTPException(status_code=404,
                            detail="Department not found")
    result = await (db[department_collection_name]
                    .delete_one({"_id": ObjectId(department_id)}))
    if result.deleted_count == 1:
...
async def create_student(student: Student):
    ...
    created_student = await (db[student_collection_name]
                             .find_one({"_id": result.inserted_id}))
    if created_student:
...
async def update_student(student_id: str, student: Student):
    ...
    if result.modified_count == 1:
        updated_student = await (db[student_collection_name]
                                 .find_one({"_id": ObjectId(student_id)}))
...
async def delete_student_by_id(student_id: str):
    db = await get_database()
    existing_student = await (db[student_collection_name]
                              .find_one({"_id": ObjectId(student_id)}))
    if existing_student is None:
        raise HTTPException(status_code=404,
                            detail="Student not found")
    result = await (db[student_collection_name]
                    .delete_one({"_id": ObjectId(student_id)}))
